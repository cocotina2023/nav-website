...
test_api "创建菜单 - 无认证" "POST" "/api/menus" '{"name":"测试菜单"}' "401"
test_api "创建菜单 - 缺少名称" "POST" "/api/menus" '{"order_index":1}' "400" "$TOKEN"
test_api "创建菜单 - 成功" "POST" "/api/menus" '{"name":"测试菜单","icon":"","order_index":1}' "200" "$TOKEN"
echo ""

echo "4. 测试卡片接口"
echo "------------------------------------------"
test_api "获取卡片列表" "GET" "/api/cards" "" "200"
test_api "创建卡片 - 无认证" "POST" "/api/cards" '{"title":"测试"}' "401"
test_api "创建卡片 - 缺少标题" "POST" "/api/cards" '{"url":"http://example.com"}' "400" "$TOKEN"
test_api "创建卡片 - 缺少URL" "POST" "/api/cards" '{"title":"测试"}' "400" "$TOKEN"
test_api "创建卡片 - 成功" "POST" "/api/cards" '{"title":"测试卡片","url":"http://example.com","menu_id":1}' "200" "$TOKEN"
echo ""

echo "5. 测试广告接口"
echo "------------------------------------------"
test_api "获取广告列表" "GET" "/api/ads" "" "200"
test_api "创建广告 - 无认证" "POST" "/api/ads" '{"image":"test.jpg"}' "401"
echo ""

echo "6. 测试友链接口"
echo "------------------------------------------"
test_api "获取友链列表" "GET" "/api/friends" "" "200"
test_api "创建友链 - 无认证" "POST" "/api/friends" '{"name":"测试"}' "401"
echo ""

echo "7. 测试用户接口"
echo "------------------------------------------"
test_api "获取用户列表 - 无认证" "GET" "/api/users" "" "401"
test_api "获取用户列表 - 有认证" "GET" "/api/users" "" "200" "$TOKEN"
echo ""
